...
def B_for_soft(shot,time, R, Z, save=False):
    ...
    for i in range(len(ax)):
        ...
        ax[i].scatter([eq.magnetic_axis.R],[eq.magnetic_axis.Z])
        ax[i].contour(R,Z,psi_rz,color='r')
    ...
    psi=psi_rz
    verBr=Br[:,0]#np.ones(np.shape(r)[1])
    verBphi=Bphi[:,0]#np.ones(np.shape(r)[1])
    verBz=Bz[:,0]#np.ones(np.shape(r)[1])
    ...
    desc='Disruption followed by RE beam triggered 48.0, time of this slice t = {} s'.format(time)

    if save:
        #save to hdf5
        listtosave = [Br, Bphi, Bz, desc, maxis.T, name, r, separatrix, wall, z, psi, verBr, verBz, verBphi]

        names = ['Br', 'Bphi', 'Bz', 'desc', 'maxis', 'name', 'r', 'separatrix', 'wall', 'z', 'Psi', 'verBr', 'verBz', 'verBphi']
        dtypes = ['f','f','f','s10','f','s10','f','f','f','f','f','f','f','f']
        ...
        filename = '/compass/home/ficker/MAGFIELD_FOR_SOFT/test{}_{}.h5'.format(shot, str(time)[:-2]+str(time)[-1])
...
B_for_soft(shot, time, R, Z, save=True)