...
#    @jit
#    @jit(nopython=True)
    def _trace_field_line(self, *coordinates, coord_type=None, sign=1, step=5e-1, t_max=150, **coords):
        """Deprecated.
        :param coordinates:
        :param coord_type:
        :param coords:
        ...
        def b_norm(t, y):
            ...
            # crd = self.coordinates(y[0], y[1], grid=False)
            # ... it should bring significant acceleration
            B = [self.B_R(R=y[0], Z=y[1], grid=False),
                 self.B_Z(R=y[0], Z=y[1], grid=False),
                 self.B_tor(R=y[0], Z=y[1], grid=False) / y[0]]
