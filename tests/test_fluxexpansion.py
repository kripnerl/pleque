...
def test_normal_vectors():
    ...
    line = Coordinates.from_coords(None, R=np.linspace(0, 1, n_points), Z=np.zeros(n_points))
    ...
    line = Coordinates.from_coords(None, R=np.linspace(0, 1, n_points), Z=np.linspace(0, 1, n_points))
    line_normals = line.normal_vector()
