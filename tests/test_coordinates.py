...
@pytest.fixture(params=[
    eq.coordinates(psi_n=np.linspace(0, 1, 30)),
    eq.coordinates(R=np.linspace(0, 1, 30), Z=np.zeros(30)),
    eq.coordinates(X=np.linspace(0, 1, 30), Y=np.linspace(0, 2, 30), Z=np.zeros(30))
])
def coords(request):
    return request.param


def create_coords_3d(*coordinates, coord_type=None, **coords):
...
def create_coords_2d(*coordinates, R=None, Z=None, coord_type=None, grid=False, **coords):
    print('--------')
...
def create_coords_1d(*coordinates, psi_n=None, coord_type=None, grid=False, **coords):
...
def test_coordinates(equilibrium):
    ...
    # 2d tests (R, Z)
    #
    coord = create_coords_2d(1, 2)
    assert coord._coord_type_input == ('R', 'Z')
    assert len(coord.x1.shape) == 1
    assert len(coord.x2.shape) == 1

    coord = create_coords_2d(R=1, Z=2)
    assert coord._coord_type_input == ('R', 'Z')
    assert len(coord.x1.shape) == 1
    assert len(coord.x2.shape) == 1

    coord = create_coords_2d([1, 2, 3, 4], [3, 5, 6, 2])
    compare_arrays([1, 2, 3, 4], coord.R)
    assert coord._coord_type_input == ('R', 'Z')
    assert len(coord.x1.shape) == 1
    assert len(coord.x2.shape) == 1

    coord = create_coords_2d(([1, 3], [3, 5], [5, 3]))
    assert coord._coord_type_input == ('R', 'Z')
    assert len(coord.x1.shape) == 1
    assert len(coord.x2.shape) == 1

    coord = create_coords_2d(np.array(([1, 3], [3, 5], [5, 3])), coord_type=['Z', 'R'])
    ...
    coord = create_coords_2d(R=[1, 2, 3, 4], Z=[3, 5, 6, 2])
    ...
    coord = create_coords_2d(r=[0.2, 0.3, 0.3, 0.2], theta=[0, np.pi / 2, np.pi, 3 / 2 * np.pi])
    ...
    # 3d case:
    coord = create_coords_3d(np.linspace(1, 5, 11), np.zeros(11), np.zeros(11))
    compare_arrays(coord.X, coord.R)
    assert len(coord.x1.shape) == 1
    assert len(coord.x2.shape) == 1

    coord = create_coords_3d(X=np.linspace(1, 5, 11), Y=np.zeros(11), Z=np.zeros(11))
    compare_arrays(coord.X, coord.R)
    assert len(coord.x1.shape) == 1
    assert len(coord.x2.shape) == 1

    coord = create_coords_3d(np.linspace(1, 5, 11), np.zeros(11), np.ones(11) * np.pi / 2)
...
def test_indexing(coords):
    coords2 = coords[::2]

    assert np.allclose(coords.x1[::2], coords2.x1)

    coords2 = coords[::-1]
    assert np.allclose(coords.x1[::-1], coords2.x1)

    coords2 = coords[4:6]
    assert np.allclose(coords.x1[4:6], coords2.x1)
